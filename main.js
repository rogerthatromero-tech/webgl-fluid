...
var mode = 0;   // 0- mouse click interaction, 1-sphere interaction (apple), 2-prism interaction
...
var quad = {};
var sphere = {};
var objRaw;     //raw primitive data for obj loading (apple)
var objModel;    //processed gl object data for obj (apple)
...
var prismCenter = vec3.create([-0.4, 0.0, 0.0]); // starting position (left of apple)
var prismRadius = 0.25;   // hit-test radius for dragging
...
function initSkyBoxTexture() {
    ...
    for (var i = 0; i < 6; i++) {
        ...
        img[i].onload = function() {
            ct++;
            if (ct == 6) {   //upon finish loading 6 images
...
// NEW: choose what we're interacting with (apple, prism, or water)
function startInteraction(x,y){
    initTracer();
    var ray = vec3.create();
    ray = rayEyeToPixel(x,y);

    // 1) Try hitting the apple (sphere) first
    ...
    if (hitSphere != null) {
        ...
        mode = 1; // move sphere/apple
        return;
    }

    // 2) Then try hitting the prism
    if (objModel2) {
        var hitPrism = rayIntersectSphere(tracer.eye, ray, prismCenter, prismRadius);
        if (hitPrism != null) {
            preHit = hitPrism;
            viewportNormal = rayEyeToPixel(gl.viewportWidth / 2.0, gl.viewportHeight / 2.0);
            vec3.negate(viewportNormal);
    ...
    // 3) Otherwise, interact with water surface
    var scale = -tracer.eye[1] / ray[1];
    var point = vec3.create([
        tracer.eye[0] + ray[0]*scale,
        tracer.eye[1] + ray[1]*scale,
        tracer.eye[2] + ray[2]*scale
    ]);
...
// NEW: move apple or prism independently
function duringInterction(x,y){

    var ray = vec3.create();
    ray = rayEyeToPixel(x,y);

    if (mode == 0) {   // direct mouse interaction on water
        var scale = -tracer.eye[1] / ray[1];
        var point = vec3.create([
            tracer.eye[0] + ray[0]*scale,
            tracer.eye[1] + ray[1]*scale,
    ...
    else if (mode == 1) {  // sphere/apple interaction
        ...
        if (vec3.length(offsetHit) > 0.0) {
            vec3.add(sphere.center, offsetHit);
            // keep sphere in pool bounds
    ...
    else if (mode == 2) {  // prism interaction
        var theEye2 = vec3.create(tracer.eye);
        var preRay2 = vec3.create(preHit);
        var nxtRay2 = vec3.create(ray);

        vec3.subtract(preRay2, theEye2);
        var t1p = vec3.dot(viewportNormal, preRay2);
        var t2p = vec3.dot(viewportNormal, nxtRay2);
        var tp = t1p / t2p;
        vec3.scale(nxtRay2, tp);

        var nxtHit2 = vec3.create();
        nxtHit2 = vec3.add(theEye2, nxtRay2);
        var offsetHit2 = vec3.create(nxtHit2);
        vec3.subtract(offsetHit2, preHit);

        if (vec3.length(offsetHit2) > 0.0) {
            vec3.add(prismCenter, offsetHit2);
            // keep prism roughly over pool
            prismCenter[0] = Math.max(-1.0, Math.min(1.0, prismCenter[0]));
            prismCenter[2] = Math.max(-1.0, Math.min(1.0, prismCenter[2]));
        }
...
function drawScene() {
    ...
    lightProj = mat4.ortho(-2,2,-2,2,-4,4);  //axis-aligned box (-2,2),(-2,2),(-4,4)
    ...
    var centerLocal = vec3.create(sphere.center);
    forwardVector = vec3.subtract(centerLocal, point);
...
function drawDepth(colTexture, depTexture, modelView, proj, model, renderColor, mode){   //draw depth from light source
    ...
    gl.enable(gl.DEPTH_TEST);
    gl.colorMask(renderColor, renderColor, renderColor, renderColor);
    gl.useProgram(depthProg);
...
function drawGodrayPass1(){
    ...
    gl.uniformMatrix4fv(godrayProg.lightMatrixUniform, false, lightMatrix);
...
function webGLStart() {
    ...
    initCustomeTexture(water.TextureC, gl.RGBA, filter, gl.FLOAT, textureSize2, textureSize2);
